# taken from home-manager's DAG library
# https://github.com/nix-community/home-manager/blob/master/modules/lib/types-dag.nix
{lib}: let
  inherit
    (lib)
    defaultFunctor
  ...
  dagEntryOf = elemType: let
    ...
    maybeConvert = def:
      if rnc.dag.isEntry def.value
      then def.value
      else
        rnc.dag.entryAnywhere (
          if def ? priority
